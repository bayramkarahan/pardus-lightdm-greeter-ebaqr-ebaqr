from ebaebaqr import EbaebaqrWidget
import json

userid_cache = ""
ebaebaqrpopover = None
q = None
...
def _ebaqr_button_event(widget):
    ebaebaqrpopover.popup()
    GLib.idle_add(q.refresh)


def module_init():
    global ebaebaqrpopover
    global q
    q = EbaebaqrWidget()
    q.data_action = qr_json_action
    ebaebaqrpopover = Gtk.Popover()
    q.set_size_request(400,550)
    ebaebaqrpopover.set_size_request(400,550)
    ebaebaqrpopover.add(q)
    button = Gtk.MenuButton(label="EBA-ebaqr", popover=ebaebaqrpopover)
